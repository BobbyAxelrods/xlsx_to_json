...
def upload_file():
    if request.method == 'POST':
        ...
        if file_extension == '.xlsx':
            converted_filename = convert_to_json_from_excel(file)
        elif file_extension == '.csv':
            converted_filename = convert_to_json_from_csv(file)
        else:
...
def convert_to_json_from_excel(file):
...
def convert_to_json_from_csv(file):
    working_path = os.getcwd()
    ...
    # Perform the conversion to JSON
    df = pd.read_csv(file_path, nrows=10, dtype=str)
    converted_filename = os.path.splitext(file.filename)[0]
    converted_file_path = os.path.join(output_folder, f"{converted_filename}.json")
    df.to_json(converted_file_path, orient='records', indent=2)
